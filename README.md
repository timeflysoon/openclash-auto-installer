...
- OpenWrt 25.12 的 `apk` 与 LuCI 界面包已适配，但这不代表官方原版内核能够运行 daed。官方原版固件通常未提供 BTF，脚本会在安装前终止，避免装完后无法启动。
- `vmlinux-btf` 不是通用依赖包，必须针对同一固件版本、目标架构和内核源码自行构建；不能直接安装其他固件或其他内核版本的包。
